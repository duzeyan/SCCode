...
void MapApp::Run(){
	...
	//启动时的初始化
	//lastID=_planPath._planPathQueue[0]-1+START_NODE_ID; //编号转ID
...
void MapApp::Intialize(const char* loadpath){
	...
	_mapFile->LoadAdjMat(_map);			//加载自建地图 邻接表
...
void MapApp::StartPlan(int& start){
	...
	double dis=0;
	double distanceThreshold=500; //启动点附近距离 单位m
...
void MapApp::PathPlaning(int s,int e){
	...
	for(size_t i=0;i<pathNode.size()-1;i++){
		...
		int tempID=0;
		int tempID2=0;
		MAP_BUTTON_NOTE	theNode1=MapTools::GetNodeByID(_map.mapNode
							,MapTools::Code2ID(pathNode[i],1));
		for(int j=0;j<theNode1.neigh;j++){ //找到相邻路口之间的道路ID
			tempID=theNode1.NeighLineID[j];
			//找到公共边
			MAP_BUTTON_NOTE	theNode2=MapTools::GetNodeByID(_map.mapNode
		...
		pathWhole.push_back(MapTools::ID2Code(tempID)); //当前道路编号
	}
	...
	for(size_t i=0;i<this->_planPath.planPathQueue.size();i++){
...
void MapApp::Dijkstra(int s,int e,vector<int> &pathV){	
	...
	for(int i=1;i<nodeCount;i++){ //迭代 n-1 次
		...
		//用新加入节点更新 U中的d值和pi
		if(newNode==-1) //找不到最近点了
			break;
		S[newNode]=1;
		for(int j=0;j<nodeCount;j++){ 
			if(S[j]!=1){  //U
				if(_map.adjMat[newNode+j*nodeCount]+d[newNode]<d[j]){ //松弛 更新d[j]和pi[j]
					pi[j]=newNode;
	...
	//code=MapTools::ID2Code(_map.mapNode[sIndex].idself);
	//pathV.push_back(code);  //等价s
...
void MapApp::Send2Mo(char buff[],int n){
		...
		//MAP_PRINT("发送MO 数据包大小%d\n",nByte);
		//MAP_PRINT("发送MO 返回值%d\n",r);
		// MapTools::ms_sleep(10);    
}
...
void MapApp::RecordWrite(int curID,int lastID){
	...
	/*for(int i=0;i<len;i++){
		int temp=_planPath.planPathQueue[i];
		fwrite(&temp,sizeof(int),1,pRecord); 
	}*/
	fwrite(&_planPath.planPathQueue[0],sizeof(int),len,pRecord);

	//GPS点
	len=_GPSList.size();
	fwrite(&len,sizeof(int),1,pRecord); 
	fwrite(&_GPSList[0],sizeof(MAP_DOUBLE_POINT),len,pRecord);
}
...
void MapApp::Release(){
	...
	_mapTaskNode.clear();  //元素被清空 内存未释放
	_GPSList.clear();      //
}
